...
function SellModal({ nft, showModal, handleModal }) {
  ...
  async function approveMarketContract() {
    try {
      const res = await tokenContract.current.methods
        .approve(config.marketContract.address, parseInt(nft.token_id))
        .send({
          from: activeAcc,
        });
      console.log(res);
  ...
  return (
    <div>
      <Modal show={showModal} onHide={handleModal}>
        ...
        >
          <Modal.Title>
            <Typography style={{ color: "white" }}>Sell your NFT</Typography>
          </Modal.Title>
        </Modal.Header>
